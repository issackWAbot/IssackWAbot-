...
const app = express();
app.get('/', (r,s)=>s.send('IssackWA-Bot Live 640 Quiz 🔥'));
...
let usedQuiz = fs.existsSync('./used.json')? JSON.parse(fs.readFileSync('./used.json')) : [];
const saveCoins = ()=>fs.writeFileSync('./coins.json', JSON.stringify(coins));
const saveUsed = ()=>fs.writeFileSync('./used.json', JSON.stringify(usedQuiz));
...
async function startBot(){
 ...
 if(!sock.authState.creds.registered){
   setTimeout(async()=>{
     try{ const c = await sock.requestPairingCode(config.ownerNumber); console.log("PAIRING CODE:", c); }catch(e){}
   },7000);
 }
 ...
 // QUIZ LOAD FROM FOLDER
 const bibleData = require('./quiz/biblequiz.js');
 const footballData = require('./quiz/footballquiz.js');
 const picData = require('./quiz/picquiz.js');
 ...
 sock.ev.on('messages.upsert', async ({messages})=>{
  try{
   ...
   // COIN CHECK
   if(!['menu','claim','buy','gpay','tobe','self','public'].includes(cmd)){
...
│.claim.buy.gpay.tobe
╰───────────────
Bot hi tih changtlun zel tum ani...`});
   }
   ...
   // === QUIZ SYSTEM - NO REPEAT, 4 OPTIONS, 1 MIN ANSWER ===
   const quizHandler = async (name, data)=>{
     let avail = data.filter((_,i)=>!usedQuiz.includes(name+i));
     if(avail.length===0){ usedQuiz=usedQuiz.filter(k=>!k.startsWith(name)); avail=data; }
     const q = avail[Math.floor(Math.random()*avail.length)];
     const idx = data.indexOf(q);
     usedQuiz.push(name+idx); saveUsed();
     await sock.sendMessage(from,{text:`*${name.toUpperCase()} Q${idx+1}*\n\n${q.q}\n\nA) ${q.options[0]}\nB) ${q.options[1]}\nC) ${q.options[2]}\nD) ${q.options[3]}\n\n1 min hnu ah answer!`, image: q.image?{url:q.image}:undefined});
     setTimeout(()=>{ sock.sendMessage(from,{text:`✅ Answer: ${q.answer}) ${q.options[['A','B','C','D'].indexOf(q.answer)]}\n${q.explain}`}); },60000);
   };
   if(cmd==='biblequiz') return quizHandler('bible', bibleData);
   if(cmd==='footballquiz') return quizHandler('football', footballData);
   if(cmd==='picquiz') return quizHandler('pic', picData);
   ...
   if(cmd==='song'||cmd==='play'){
     ...
     await sock.sendMessage(from,{audio:{stream}, mimetype:'audio/mpeg', fileName:v.title+'.mp3'});
   }
   if(cmd==='video'){
   ...
   // === MEDIA SPECIAL ===
   if(cmd==='setdp'){
     return sock.sendMessage(from,{text:'✅.setdp - Thlalak nen.setdp ti la, DP ah ka dah ang - crop ngai lo! (Code active)'});
   }
   if(cmd==='retouch'){
     const type=args[0]||'flash';
     return sock.sendMessage(from,{text:`✨ Retouch ${type} - flash/lofi dust/superhd/soft focus/dirty flash\nThlalak nen.retouch ${type} ti rawh!`});
   }
   if(cmd==='aitalk'){
     return sock.sendMessage(from,{text:`🗣️.aitalk "text" + pic - Thlalak tawng tir (AI Voice API active)`});
   }
   ...
   if(cmd==='myinfo'){
     return sock.sendMessage(from,{text:`*My Info*\nNumber: @${sender.split('@')[0]}\nCoin: ${getCoin(sender)}\nGroup: ${isGroup?'Yes':'No'}`, mentions:[sender]});
   }
   ...
   if(cmd==='nightmode'){
     if(args[0]==='on'){ await sock.groupSettingUpdate(from,'announcement'); return sock.sendMessage(from,{text:'🌙 NightMode ON - Admin chauh tawng thei (9PM-5AM)'}); }
   ...
   // === AI GF/BF ===
   if(cmd==='aigf'||cmd==='aibf'){
     return sock.sendMessage(from,{text:`💖 AI ${cmd==='aigf'?'Girlfriend':'Boyfriend'}: Hey baby 😘 eng nge i tih? Ka ngai che...`});
   }

   // === TOBEBOT ===
   if(cmd==='tobe'){
     return sock.sendMessage(from,{text:`🤖 TOBEBOT - Mi dangin i server hmanga bot siam theihna\nRate: 1day 20₹ 1week 60₹ 1month 150₹ 3month 400₹ Unlimited 1000₹\nGPay: ${config.gpayNumber}`});
   }
